...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    // console.error('Error searching tracks:', error);*/ // possible future implementation that allows people to seach for a track
   
    // Extract the first track
    const firstTrack = response.data.tracks.items[0];
    if (!firstTrack) {
      return res.status(404).send({success: false, message: 'No tracks found.'});
    }
    // Extract the Spotify link
    const spotifyLink = firstTrack.external_urls.spotify;
   return res.status(200).send({data:spotifyLink});
 } catch (error) {
